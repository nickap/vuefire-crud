            ...
            >
              <div class="flex items-center justify-between">
                <DialogTitle class="text-xl font-medium text-gray-700">
                  {{ author.id ? 'Update' : 'Create' }} Author
                </DialogTitle>
              </div>
              <p class="mt-1 text-sm text-gray-500">
                {{
                  author.id ? 'Update this author' : 'Create a new author here'
                }}
              </p>
              <!-- Form -->
...
const submitAuthor = handleSubmit(async (values) => {
  ...
  if (!author.value.id) {
    await authorStore.add({ name: values.name });
    closeModal();
  } else {
    authorStore.update(author.value.id, { name: values.name });
    closeModal();
  }
}, onInvalidSubmit);
const open = ref(false);
const openModal = (item: IAuthor) => {
  ...
  resetForm();
  if (item) {
    author.value = JSON.parse(JSON.stringify(item));
  } else {
    author.value = {};
  }
  open.value = true;
