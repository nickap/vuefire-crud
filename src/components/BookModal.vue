      ...
      <div class="fixed inset-0 overflow-y-auto">
        ...
        <div class="flex items-center justify-center min-h-full p-4">
          ...
          >
            ...
            >
              <div v-if="book" class="flex items-center justify-between">
              ...
              <p class="mt-1 text-sm text-gray-500">
                {{ book.id ? 'Update this book' : 'Create a new book here' }}
              </p>

              <form class="mt-5" @submit="submitBook">
                <div v-if="book" class="grid grid-cols-1 gap-4 md:grid-cols-2">
...
import FormMultiAuthorsInput from '@/components/Form/MultiAuthorsInput.vue';
...
const onInvalidSubmit = () => {
  ...
  if (!submitBtn) {
    console.warn(
      "submitBtn = document.querySelector('.btn.submit'). Does the class exist?"
    );
    return;
  }
...
const submitBook = handleSubmit(async (values) => {
  if (!book.value) {
    console.log('book.value is a falsy value');
    return;
  }
  if (!book.value.id) {
    await bookStore.add({ ...values, authors: authors.value });
    closeModal();
  } else {
    bookStore.update(book.value.id, { ...values, authors: authors.value });
    closeModal();
  }
}, onInvalidSubmit);

const open = ref(false);
const openModal = (item: IBook) => {
  ...
  if (item) {
    book.value = JSON.parse(
      JSON.stringify({
        ...item,
        published: item.published.substring(0, 10),
      })
    );
